...
def part1(guard_log):
    """Solution to part 1"""
    guard_dict = make_guard_dict(guard_log)
...
def part2(guard_log):
    ...
    for hour, minute in itertools.product(range(24), range(60)):
        for guard, times in guard_dict.items():
            for _, start, end in times:
                if start[3] <= hour < end[3] or\
                        (start[3] == end[3] and start[4] <= minute < end[4]) and\
                        not (hour == end[3] and minute == end[4]):
